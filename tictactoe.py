...
#Use while loops to continually ask until we get a correct answer
def player_input():
    marker = ''
...
#Write a function that asks for a player's next position (as a number 1-9) and then uses the function space_check to check if its a free position. If it is, then return the position for later use.
...
#Now using while loops and the functions we have defined we will run the game
while True:
    # Reset the board
    theBoard = [' '] * 10
